...
export async function runAutonomousPipeline(options: AutonomousRunOptions = {}): Promise<{
  ...
  for (const signal of generationTargets) {
    ...
    const generationInput = toGenerationInputFromSignal(currentSignal);
    if (!generationInput) {
      heldRecords.push(
        buildAutonomousRecord({
          signal: currentSignal,
          stage: "auto_generate",
          decision: "hold",
          summary: "Held before generation because the generation input is incomplete.",
          reasons: ["Generation input incomplete"],
          confidenceLevel: guidance.confidence.confidenceLevel,
          strongestCaution: "Generation input incomplete",
          persisted: false,
        }),
      );
      continue;
    ...
    const generationRun = await generateDrafts(generationInput);
    const savedGeneration = await saveSignalWithFallback(currentSignal.recordId, {
    ...
    if (!savedGeneration.signal) {
      ...
      heldRecords.push(
        buildAutonomousRecord({
          ...
          stage: "auto_generate",
          decision: "hold",
          summary: "Held during generation because the generated drafts could not be saved.",
          reasons: ["Generation save failed"],
          confidenceLevel: guidance.confidence.confidenceLevel,
          strongestCaution: "Generation save failed",
          persisted: false,
        }),
      );
      continue;
  ...
  const result = autonomousRunSummarySchema.parse({
    ...
    message:
      approvalReadyRecords.length > 0
        ? `Autonomous run prepared ${approvalReadyRecords.length} approval-ready candidate${approvalReadyRecords.length === 1 ? "" : "s"}, auto-interpreted ${autoInterpreted.length}, auto-generated ${autoGenerated.length}, and held ${heldRecords.length} for human judgement.`
        : `Autonomous run completed with ${autoInterpreted.length} auto-interpreted, ${autoGenerated.length} auto-generated, and ${heldRecords.length} held cases. No approval-ready candidates surfaced this pass.`,
  });
