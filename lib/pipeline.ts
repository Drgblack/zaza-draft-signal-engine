...
const evergreenRunRecordSchema = z.object({
  id: z.string().trim().min(1),
  signalId: z.string().trim().min(1),
  sourceTitle: z.string().trim().min(1),
  surfacedPlatform: z.enum(["x", "linkedin", "reddit"]),
  priorPostDate: z.string().trim().min(1),
  priorOutcomeQuality: z.enum(["strong", "acceptable", "weak"]),
  priorReuseRecommendation: z.enum(["reuse_this_approach", "adapt_before_reuse", "do_not_repeat"]),
  reuseMode: z.enum(["reuse_directly", "adapt_before_reuse"]),
  strategicValue: z.enum(["high", "medium", "low", "unclear"]).nullable(),
  reasons: z.array(z.string().trim().min(1)).max(3),
  weeklyGapReasons: z.array(z.string().trim().min(1)).max(3),
  rankScore: z.number(),
});
...
export const autonomousRunSummarySchema = z.object({
  ...
  evergreenIdentified: z.number().int().nonnegative(),
  evergreenSuppressed: z.number().int().nonnegative(),
  evergreenSurfaced: z.number().int().nonnegative(),
  ...
  records: z.object({
    ...
    evergreenSurfaced: z.array(evergreenRunRecordSchema),
  ...
  topEvergreen: z.array(evergreenRunRecordSchema),
  errors: z.array(pipelineErrorSchema),
  touchedRecordIds: z.array(z.string().trim().min(1)),
  message: z.string().trim().min(1),
});
...
export async function runAutonomousPipeline(options: AutonomousRunOptions = {}): Promise<{
  ...
  const topEvergreen = evergreenSummary.candidates.map((candidate) =>
    evergreenRunRecordSchema.parse({
      id: candidate.id,
      signalId: candidate.signalId,
      sourceTitle: candidate.signal.sourceTitle,
      surfacedPlatform: candidate.surfacedPlatform,
      priorPostDate: candidate.priorPostDate,
      priorOutcomeQuality: candidate.priorOutcomeQuality,
      priorReuseRecommendation: candidate.priorReuseRecommendation,
      reuseMode: candidate.reuseMode,
      strategicValue: candidate.strategicValue,
      reasons: candidate.reasons,
      weeklyGapReasons: candidate.weeklyGapReasons,
      rankScore: candidate.rankScore,
    }),
  );

  for (const candidate of evergreenSummary.candidates.slice(0, 5)) {
    auditEvents.push({
      signalId: candidate.signalId,
      eventType: "EVERGREEN_CANDIDATE_IDENTIFIED",
      actor: "system",
      summary: `Evergreen candidate identified from prior ${candidate.surfacedPlatform} post.`,
      metadata: {
        evergreenCandidateId: candidate.id,
        priorOutcomeQuality: candidate.priorOutcomeQuality,
        reuseMode: candidate.reuseMode,
        rankScore: candidate.rankScore,
      },
    ...
    auditEvents.push({
      signalId: candidate.signalId,
      eventType: "EVERGREEN_RESURFACED",
      actor: "system",
      summary: candidate.reasons[0] ?? "Evergreen winner resurfaced for review.",
      metadata: {
        evergreenCandidateId: candidate.id,
        surfacedPlatform: candidate.surfacedPlatform,
        priorPostDate: candidate.priorPostDate,
  ...
  for (const suppressedCandidate of evergreenSummary.suppressed.slice(0, 5)) {
    auditEvents.push({
      signalId: suppressedCandidate.signalId,
      eventType: "EVERGREEN_SUPPRESSED",
      actor: "system",
      summary: suppressedCandidate.reasons[0] ?? "Evergreen candidate suppressed.",
      metadata: {
        surfacedPlatform: suppressedCandidate.surfacedPlatform,
        priorPostDate: suppressedCandidate.priorPostDate,
        priorOutcomeQuality: suppressedCandidate.priorOutcomeQuality,
      },
    });
  }
  ...
  const result = autonomousRunSummarySchema.parse({
    ...
    evergreenIdentified: evergreenSummary.eligibleCount,
    evergreenSuppressed: evergreenSummary.suppressedCount,
    evergreenSurfaced: evergreenSummary.surfacedCount,
    ...
    records: {
      ...
      evergreenSurfaced: topEvergreen,
    ...
    message:
      approvalReadyRecords.length > 0
        ? `Autonomous run prepared ${approvalReadyRecords.length} approval-ready candidate${approvalReadyRecords.length === 1 ? "" : "s"}, auto-interpreted ${autoInterpreted.length}, auto-generated ${autoGenerated.length}, resurfaced ${evergreenSummary.surfacedCount} evergreen candidate${evergreenSummary.surfacedCount === 1 ? "" : "s"}, repaired ${repairedPromoted.length} held candidate${repairedPromoted.length === 1 ? "" : "s"}, and held ${heldRecords.length} for human judgement.`
        : `Autonomous run completed with ${autoInterpreted.length} auto-interpreted, ${autoGenerated.length} auto-generated, ${evergreenSummary.surfacedCount} evergreen resurfaced, ${repairedPromoted.length} repaired promotions, and ${heldRecords.length} held cases. No approval-ready candidates surfaced this pass.`,
  });
