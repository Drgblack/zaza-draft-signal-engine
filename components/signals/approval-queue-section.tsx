        ...
        <CardContent className="space-y-3.5">
          <div className="grid gap-3 lg:grid-cols-3">
            {REVIEW_COMMAND_CENTER_VIEWS.filter((view) => ["all", "ready_to_approve", "needs_judgement", "stale", "missing_outcomes", "experiment_linked", "fatigued", "campaign_critical", "auto_repaired"].includes(view.id)).map((view) => (
              <button key={view.id} type="button" onClick={() => setActiveView(view.id)} className={`rounded-2xl border px-4 py-3 text-left transition ${activeView === view.id ? "border-[color:var(--accent)]/40 bg-white shadow-sm" : "border-black/5 bg-white/70 hover:bg-white"}`}>
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            {(
              ...
              <div key={state} className="rounded-2xl border border-black/5 bg-slate-50/80 px-4 py-3.5">
                <div className="flex items-center justify-between gap-3">
          ...
          {staleFeedback ? (
            <div className="rounded-2xl border border-black/5 bg-slate-50/80 px-4 py-4 text-sm leading-6 text-slate-500">{staleFeedback}</div>
          ) : null}
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
            <span className="rounded-full bg-slate-100 px-2.5 py-1">Shortcuts</span>
            <span>`j` next</span>
            <span>`k` previous</span>
            <span>`e` expand</span>
            <span>`o` open final review</span>
            <span>`?` help</span>
            <button type="button" onClick={() => setShowShortcutHelp((current) => !current)} className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-700 transition hover:bg-slate-200">{showShortcutHelp ? "Hide help" : "Show help"}</button>
          </div>
          {showShortcutHelp ? <div className="rounded-2xl border border-black/5 bg-slate-50/80 px-4 py-4 text-sm leading-6 text-slate-500"><p className="font-medium text-slate-900">Queue shortcuts</p><p className="mt-1.5">`j` next candidate · `k` previous candidate · `e` expand or collapse detail · `o` open focused candidate in final review · `?` toggle help</p></div> : null}
          {filteredCandidates.length === 0 ? <div className="rounded-2xl border border-black/5 bg-white/76 px-4 py-4 text-sm leading-6 text-slate-500">No approval-ready candidates surfaced yet.</div> : filteredCandidates.map((candidate, index) => {
            ...
            return (
              <div key={candidate.signal.recordId} className={`rounded-2xl px-4 py-3.5 transition ${activeIndex === index ? "bg-white ring-2 ring-[color:var(--accent)]/30" : "bg-white/80"}`}>
                <div className="flex flex-col gap-3.5 xl:flex-row xl:items-start xl:justify-between">
                  <div className="min-w-0 flex-1 space-y-2.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {chips.map((chip) => <span key={chip} className="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{chip}</span>)}
                      {candidate.stale.state !== "fresh" ? (
                        <>
                          <span className="inline-flex rounded-full bg-rose-50 px-2.5 py-1 text-xs font-medium text-rose-700">{candidate.stale.summary}</span>
                          <span className="inline-flex rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">Action: {candidate.stale.actionSummary}</span>
                        </>
                      ) : null}
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {shouldShowStaleActions(candidate) ? (
                        <>
                          <button
                            type="button"
                            className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-sm font-medium text-amber-700 transition hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60"
                          ...
                          <button
                            type="button"
                            className="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-1 text-sm font-medium text-sky-700 transition hover:bg-sky-100 disabled:cursor-not-allowed disabled:opacity-60"
                          ...
                          <button
                            type="button"
                            className="inline-flex items-center rounded-full bg-rose-50 px-2.5 py-1 text-sm font-medium text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
                          ...
                          <button
                            type="button"
                            className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-1 text-sm font-medium text-slate-700 transition hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-60"
                  ...
                  <div className="w-full max-w-sm rounded-2xl border border-black/5 bg-slate-50/80 px-4 py-3.5 text-xs leading-5 text-slate-500 xl:w-80">
                    <p>{candidate.assessment.suggestedPlatformPriority ?? candidate.signal.platformPriority ?? "Platform not set"}</p>
                    <p className="mt-1.5">{candidate.signal.editorialMode ? getEditorialModeDefinition(candidate.signal.editorialMode).label : "Editorial mode not set"}</p>
                    <p className="mt-1.5">Conversion posture: {conversionIntentLabel(candidate.conversionIntent.posture)}</p>
                    <p className="mt-1.5">Distribution: {candidate.distributionPriority.primaryPlatformLabel} · {distributionStrategyLabel(candidate.distributionPriority.distributionStrategy)}</p>
                    ...
                    <p className="mt-1.5">{formatContextLabel(candidate.signal.campaignId) ?? "No campaign"}</p>
                    <p className="mt-1.5">{formatContextLabel(candidate.signal.funnelStage) ?? "Funnel not set"} · {formatContextLabel(candidate.signal.ctaGoal) ?? "CTA not set"}</p>
                    <p className="mt-1.5">{candidate.commercialRisk.topRisk?.reason ?? candidate.triage.reason ?? candidate.automationConfidence.reasons[0] ?? candidate.conflicts.topConflicts[0]?.reason ?? (candidate.stale.state !== "fresh" ? candidate.stale.reasons[0]?.summary ?? candidate.assessment.strongestCaution ?? candidate.guidance.cautionNotes[0] ?? "No major caution surfaced" : candidate.assessment.strongestCaution ?? candidate.guidance.cautionNotes[0] ?? "No major caution surfaced")}</p>
                    {candidate.preReviewRepair.decision === "applied" ? <p className="mt-1.5 text-xs text-slate-500">{candidate.preReviewRepair.summary}</p> : null}
                    {candidate.executionChain.status === "completed" || candidate.executionChain.status === "available" ? <p className="mt-1.5 text-xs text-sky-700">{candidate.executionChain.summary}</p> : null}
                    {candidate.commercialRisk.topRisk ? <p className="mt-1.5 text-xs text-rose-700">Suggested fix: {candidate.commercialRisk.topRisk.suggestedFix}</p> : null}
                    {candidate.stale.operatorActionNote ? <p className="mt-1.5 text-xs text-slate-500">Note: {candidate.stale.operatorActionNote}</p> : null}
                  </div>
                </div>
    ...
    <div id="evergreen-resurfacing">
      <Card>
        ...
        <CardContent className="space-y-4">
          {candidates.length === 0 ? <div className="rounded-2xl border border-black/5 bg-white/76 px-4 py-4 text-sm leading-6 text-slate-500">No evergreen candidates are currently strong enough to surface.</div> : candidates.map((candidate) => (
            <div key={candidate.id} className="rounded-2xl border border-black/5 bg-white/80 px-4 py-3.5">
              <div className="flex flex-col gap-3.5 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-2.5">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-slate-500">{[...candidate.reasons, ...candidate.weeklyGapReasons].slice(0, 4).map((reason) => <span key={reason} className="rounded-full bg-slate-100 px-2.5 py-1">{reason}</span>)}</div>
                  <div className="flex flex-wrap gap-2">
                ...
                <div className="grid gap-2.5 rounded-2xl border border-black/5 bg-slate-50/80 px-4 py-3.5 text-xs leading-5 text-slate-500 lg:w-[360px]">
        ...
        <CardContent className="space-y-4">
          {candidates.length === 0 ? (
            ...
            candidates.map((candidate) => (
              <div key={candidate.signal.recordId} className="rounded-2xl border border-black/5 bg-white/80 px-4 py-3.5">
                <div className="flex flex-col gap-3.5 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2.5">
                    <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                      {candidate.rankReasons.slice(0, 2).map((reason) => (
                        <span key={reason} className="rounded-full bg-slate-100 px-2.5 py-1">
                          {reason}
                        </span>
                      ))}
                    </div>
