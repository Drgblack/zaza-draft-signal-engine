...
test("visual provider registry exposes runway, kling, and veo candidates", () => {
  const providers = listVisualProviders().map((provider) => provider.id);

  assert.deepEqual(providers, ["runway-gen4", "kling-2", "veo-3"]);
});
...
test("visual provider resolver keeps backward compatibility for local-default", () => {
  ...
  assert.equal(getVisualProvider("veo-3").id, "veo-3");
